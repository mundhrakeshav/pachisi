...
//      JobID: a7ab70d561d34eb49e9b1612fd2e044b
    ...
    constructor(string memory _url, uint _betResolveTime) public {
    ...
    function claimableFunds(address _userAddress) public view returns(uint){
        if(keccak256(abi.encodePacked(symbol)) == keccak256(abi.encodePacked(greaterSymbol)) && predictionPrice > resolvedPrice) {
        ...
        if(keccak256(abi.encodePacked(symbol)) == keccak256(abi.encodePacked(lessSymbol)) && predictionPrice < resolvedPrice) {
            return userOwnedTrueTokens[_userAddress] * volume/trueTokensInMarket;
        } else {
            return userOwnedFalseTokens[_userAddress] * volume/falseTokensInMarket;    
        }
