...
contract CricketGame is ChainlinkClient, EIP712MetaTransaction("CricketPachisi", "1"){
   ...
   function addCricketTossBet(  
        ...
        address _userAddress) 
            public {
             if(_predictedOutcome) {
                totalVolumeOnTossOutcome1 += _betAmount;
                betAmountsTossOutcome1[_userAddress] += _betAmount;
            } else {
                totalVolumeOnTossOutcome2 += _betAmount;
                betAmountsTossOutcome2[_userAddress] += _betAmount;
        }
    }
    ...
    function claimUserFunds(address _userAddress) public {
        betAmountsMatchOutcome2[_userAddress] = 0;
        betAmountsMatchOutcome1[_userAddress] = 0;
        betAmountsTossOutcome1[_userAddress] = 0;
        betAmountsTossOutcome2[_userAddress] = 0;
        }
}
